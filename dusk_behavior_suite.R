...
dusk_behavior_suite <- function(dt){
                             ...
                             ifelse(t %between% c(hours(6), hours(9)), "base after", 
                                    ifelse(t %between% c(hours(9), hours(12)), "base eve", 
                                           ifelse(t %between% c(hours(12), hours(15)), "base twilight", 
                                                  ifelse(t %between% c(hours(15), hours(18)), "base night", 
                                                      ifelse(t %between% c(hours(18), hours(21)), "base midnight", 
                                                        ifelse(t %between% c(hours(21), hours(24)), "base anticip",
                                                            ifelse(t %between% c(hours(24), hours(30)), "activate morn", 
                                                                ifelse(t %between% c(hours(30), hours(33)), "activate after", 
                                                                       ifelse(t %between% c(hours(33), hours(36)), "activate eve", 
                                                                              ifelse(t %between% c(hours(36), hours(39)), "activate twilight",  
                                                                                     ifelse(t %between% c(hours(39), hours(42)), "activate night", 
                                                                                            ifelse(t %between% c(hours(42), hours(45)), "activate midnight", "activate anticip")))))))))))))]
  ...
  dt_summ <- rejoin(dt[,
                       .(
                         ...
                         sleep_sum_base_night = sum(asleep[phase %in% c("base twilight", "base night", "base midnight", "base anticip")]), 
                         sleep_sum_base_anticip = sum(asleep[phase == "base anticip"]),
                         ...
                         sleep_sum_activ_night = sum(asleep[phase %in% c("activate twilight", "activate night", "activate midnight",  "activate anticip")]), 
                         sleep_sum_activ_anticip = sum(asleep[phase == "activate anticip"]),
                         ...
                         activity_sum_base_night = sum(activity[phase %in% c("base twilight", "base night", "base midnight", "base anticip")]), 
                         ...
                         activity_sum_activ_night = sum(activity[phase %in% c("activate twilight", "activate night","activate midnight", "activate anticip")]), 
